...
7. **Verify before done — objective gate is MANDATORY, not optional.** Run `AGENTS.md` § Objective verification Step 1 (pipeline EXIT 0) + Step 2 (golden snapshot EXIT 0). If either fails, the chunk is NOT done — fix and re-run. **Cross-platform:** if claiming platform parity, test on Windows + Linux (WSL counts); if a platform can't be exercised, surface that honestly.
