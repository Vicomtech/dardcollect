...
def scene_changed(
    ...
    **Signal 3 – Spatial block-delta (opt-in, issue #4)**
    Same-set shot/reverse-shot cuts survive signal 1 (the global histogram is
    spatially invariant) and need no ≥4× person-area jump for signal 2. The
    block-delta compares mean luminance per cell of a downscaled 4×4 grid and
    fires when a majority of cells changed — exactly the same-set re-framing
    case. Gated by *block_delta* (default off pending calibration).

    A scene change is declared when any enabled signal fires.
