...
def _load_resume_start(progress_path: Path, total_frames: int, cap) -> int:
...
def _build_frame_data(tracklets_kpts: list) -> list[dict]:
...
def _save_progress(progress_path: Path, frame_id: int, video_path: Path) -> None:
    try:
...
def process_video(
    ...
    start_frame = _load_resume_start(progress_path, total_frames, cap)
    ...
    while True:
        ...
        # Drop bboxes covering too large a fraction of the frame (title cards,
        # overlays) or with extreme aspect ratio (furniture, animals, not persons).
        det_bboxes, det_scores = _filter_detections(
        ...
        if tracklets:
            ...
            face_visible, mouth_open = _compute_face_flags(
                tracklets_kpts, height, clip_config, det_config, poser
            )
            current_frame_data = _build_frame_data(tracklets_kpts)
        ...
        if _should_progressive_flush(
            ...
            pending_segments = []
            frames_since_flush = 0
            _save_progress(progress_path, frame_id, video_path)
