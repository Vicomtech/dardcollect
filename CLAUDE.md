...
2. `uv run python scripts/golden_snapshot.py --dard-root DARD_test compare tests/fixtures/golden_manifest.json --validate` — exits non-zero on any added/removed/changed CSV/sidecar vs the baseline OR any schema-invalid sidecar.
...
- **Validate-at-write contract:** `add_fair_metadata` + `validate_against_schema` are called at write for document, transcription, face-crop (image + video), and image-detection sidecars. `face_crop_schema.json` is a `oneOf` of a video variant (requires `source_video`/`track_id`/`duration_seconds`) and an image variant (requires `image_path`/`person_idx`); `image_detection_schema.json` covers the per-image detection sidecar. Not yet validated at write: `quality_annotation` (no ratified outputs to confirm conformance against) — tracked debt.
