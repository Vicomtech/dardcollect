...
- **Tests:** a small CPU-only unit suite exists under `tests/` (`test_fair.py` — FAIR metadata + JSON-Schema validation; `test_config.py` — config parsing + log-level). Run with `uv run python -m pytest tests/ -q` (34 tests, ~3s, no GPU needed). **Note:** `pytest` is referenced in [docs/4-DEVELOPMENT.md](docs/4-DEVELOPMENT.md) but is NOT yet declared in `[project.optional-dependencies] dev` (only `ruff` + `ty` are) — it must be installed in the venv (`uv pip install pytest`) until added to the `dev` extra. The suite covers only the pure CPU helpers; GPU-accelerated stages (detection/pose/OCR/quality) are still verified via the snapshot/golden + CSV/JSON-sidecar harness below, not unit tests.
