...
from dardcollect.frames import FrameRequest, extract_frames
...
@dataclass
class _FrameJob:
    """Clip-mode shared state: fixed config for one extract_frames call."""
...
def _prepare_clip_frame_run(cfg, input_dir, output_dir):
    """Init logger + discover clip videos (fail loud when the input is empty)."""
    ...
    return frames_logger, video_files, clip_type, workers


def _extract_one(job: _FrameJob, video_path: Path) -> None:
    """Extract PNG frames from one clip video (per-source-subdir layout)."""
    ...
    rel_parent = video_path.relative_to(job.input_dir).parent
    video_output_dir = job.output_dir / rel_parent / video_path.stem
    ...
    extract_frames(
        FrameRequest(
            ...
            overwrite=job.cfg.overwrite,
            frames_logger=job.frames_logger,
        )
    )


def _run_parallel_frames(job: _FrameJob, video_files: list[Path], workers: int) -> None:
    """Extract frames with a thread pool (per-file resume; fail loud when full)."""
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {pool.submit(_extract_one, job, v): v for v in video_files}
        try:
            for future in tqdm(
                ...
                check_disk_space(job.output_dir, job.cfg.min_free_disk_gb)
        except SystemExit:
...
def main(config_path: str | None = None) -> None:
    ...
    frames_logger, video_files, clip_type, workers = _prepare_clip_frame_run(
    ...
    if workers > 1:
        _run_parallel_frames(job, video_files, workers)
    else:
        for video_path in tqdm(video_files, desc="Extracting frames", unit="video"):
            check_disk_space(output_dir, cfg.min_free_disk_gb)
            _extract_one(job, video_path)
