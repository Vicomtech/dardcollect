...
- **Platform verification:** a chunk is not done until it works on Windows AND Linux — the two platforms the pipeline is verified on. Run on at least the platforms you have available (WSL counts as Linux); if one can't be exercised, surface that honestly rather than marking done off a single-platform pass.
- **Progressive rerun gating:** after a first successful run, a downstream stage waits for real dependency updates (or dependency finish) before re-launching — it does NOT re-launch on timeout alone. This is implemented via `_dependency_update_gate` in `scripts/run_pipeline.py`. When writing tests that mock `_run_stage_once`, the mock MUST update `state.runs` to mirror the real side-effect; failing to do so causes `_dependency_update_gate` to always see `runs == 0` and spin the worker in an infinite loop.
- **Per-file SKIP verbosity:** `SKIP (already done)` per-video log lines are intentionally at `DEBUG` level (not `INFO`) in `pipeline/extract_face_crops_from_videos.py` and `pipeline/extract_person_clips_from_videos.py`. A single aggregated `Resume: skipped N already-processed video(s)` line is emitted at `INFO`. Do not revert this to per-file INFO — it floods the terminal during progressive reruns.
- **local_cache_dir cleanup:** when `person_extraction.preload_source_to_local: true`, each video's local copy is deleted after processing. If `local_cache_dir` is explicitly configured (not a system temp dir), the directory is also removed when empty. See `_remove_local_copy` in `dardcollect/person_clips.py`.
...
3. **Implementation gates** (§ § Objective verification § Chunk DONE ✅ applies)
   ...
   - Platform testing: Windows + (WSL | Linux)
...
- ✅ Every gate in § Objective verification is green: CPU (ruff check + format, ty check, pytest), code-quality ratchet (no NEW/WORSENED/UNCODIFIED violation; every pinned entry justified by an `EXCEPTION_RULES` rule), size (files ≤ 600, functions ≤ 80 code lines, no god-file grown), circular deps (0), dead-code pruned, documentation synced (README + AI Systems + sub-docs + config files), objective gate (pipeline EXIT 0 + golden compare EXIT 0)
...
1. **Filtering (bounded inputs/outputs):**
   - Never read a full file when only a function/section is needed. Use `Read` with `offset`/`limit`; use Grep with context/limit to land on the exact symbol before reading. Big files in this repo (`quality.py`, `run_pipeline.py`, `orchestrator_plan.py`, `person_clips.py`) make full reads expensive — narrow first.
   ...
   - Prefer targeted Grep/Glob with include-filters over broad fan-outs; resolve each hit with a narrow `Read` before deciding.
...
- **Harness-change rejection memory**: a proposed-but-rejected harness change is recorded as a
  MEMORY.md Open item with the reason AND its reopening condition (the concrete event that
  legitimizes re-proposing it — e.g. the tool being registered, a repeated incident). The same
  change must not be silently re-proposed in a later session; a re-proposal is legitimate only
  when the recorded condition has fired. (Existing instance: `socraticode-index-first` retired
  2026-09-10 — reopening condition: a SocratiCode MCP server is actually registered in
  kilo.json.)