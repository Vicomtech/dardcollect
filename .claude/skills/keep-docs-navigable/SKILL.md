...
3. **Links must resolve.** Use relative links from the repo root. After adding/renaming/moving a doc, check that every inbound link still resolves (grep for the old path). A broken docs index is worse than no index. **Mind the depth of the file you're editing:** a doc at `docs/X.md` links a sibling as `Y.md` and a repo-root file as `../Z.md`; a skill at `.claude/skills/<name>/SKILL.md` (3 levels deep) links repo-root docs as `../../../docs/Y.md` and `CLAUDE.md` as `../../../CLAUDE.md`. Getting the `../` count wrong is the commonest broken-link cause here.
