...
- **Code-quality + dead-code ratchet**: `scripts/quality_gates.py` collects cyclomatic complexity > 10 (`C901`), functions > 80 lines, too many args/branches/statements (`PLR0913/0912/0915`), unused parameters (`ARG`), bugbear `B`, and vulture dead code (≥ 60% confidence), then compares against the user-owned `scripts/quality_baselines.json`. A NEW or WORSENED violation fails; a resolved/improved entry prints a "lower the baseline" note. The agent never raises a baseline. (These ruff rules are deliberately not in `select` — turning them on would fail on the frozen debt; the ratchet is the enforcement. `tests/*` is exempt from `ARG`.)
